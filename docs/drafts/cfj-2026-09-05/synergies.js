...
// C: 異種の採用で既存特性の入口を開く。付与は戦闘コピーだけに限定する。
SYNERGIES.push(
  {
    id: "slime_collection", name: "粘着集金班",
    condition: "軍団にゴブリン3体＋スライム2体、スライムが出撃",
    desc: "出撃スライムに追い剥ぎ。初ダメージで1G予約→味方の強欲へ接続",
    check(units, ctx) {
      const p = Synergy.pool(units, ctx);
      return p.filter(u => u.race === "ゴブリン").length >= 3 && p.filter(u => u.race === "スライム").length >= 2 && units.some(u => u.race === "スライム");
    },
  ...
  },
  {
    ...
    condition: "軍団に魔法職2体＋アンデッド3体、火球持ちが出撃",
    desc: "火球が敵全体へ広がる。撃破の余剰はOVERKILLと戦意へ",
    check(units, ctx) {
      const p = Synergy.pool(units, ctx);
      return p.filter(u => u.tags.includes("caster")).length >= 2 && p.filter(u => u.tags.includes("undead")).length >= 3 && units.some(u => u.traits.includes("fireball"));
    },
    apply(units) { for (const u of units) if (u.traits.includes("fireball")) u.mods.fireballAll = true; }
  },
  {
    id: "hound_audit", name: "嗅ぎつける監査",
    condition: "軍団にコボルト2体＋ゴブリン3体、コボルトが出撃",
    desc: "出撃コボルトに強欲。味方の金貨獲得に威力70%で追撃（同じ連鎖で各1回）",
    check(units, ctx) {
      const p = Synergy.pool(units, ctx);
      return p.filter(u => u.race === "コボルト").length >= 2 && p.filter(u => u.race === "ゴブリン").length >= 3 && units.some(u => u.race === "コボルト");
    },
    apply(units) { for (const u of units) if (u.race === "コボルト" && !u.traits.includes("greedy")) u.traits.push("greedy"); }
  },
  {
    id: "grave_shift", name: "夜勤の引き継ぎ",
    condition: "軍団にゾンビ2体＋骸骨兵3体、ゾンビが出撃",
    desc: "出撃ゾンビに死霊術。倒れた同僚を蘇生→魂の徴収・殉職手当へ",
    check(units, ctx) {
      const p = Synergy.pool(units, ctx);
      return p.filter(u => u.race === "ゾンビ").length >= 2 && p.filter(u => u.race === "骸骨兵").length >= 3 && units.some(u => u.race === "ゾンビ");
    },
  ...
  },
  {
    ...
    condition: "軍団にオーク2体＋インプ3体、オークが出撃",
    desc: "出撃オークに追い剥ぎ。初ダメージの1G予約で強欲や恐喝帳簿を動かす",
    check(units, ctx) {
      const p = Synergy.pool(units, ctx);
      return p.filter(u => u.race === "オーク").length >= 2 && p.filter(u => u.race === "インプ").length >= 3 && units.some(u => u.race === "オーク");
    },
  ...
  },
  {
    ...
    condition: "軍団にオーガ1体＋ゴブリン4体、オーガが出撃",
    desc: "出撃オーガに強欲。味方の金貨獲得に威力70%で追撃（同じ連鎖で各1回）",
    check(units, ctx) {
      const p = Synergy.pool(units, ctx);
      return p.some(u => u.race === "オーガ") && p.filter(u => u.race === "ゴブリン").length >= 4 && units.some(u => u.race === "オーガ");
    },
    apply(units) { for (const u of units) if (u.race === "オーガ" && !u.traits.includes("greedy")) u.traits.push("greedy"); }
  },
  {
    id: "imp_salvage", name: "遺品回収係",
    condition: "軍団にインプ3体＋アンデッド2体、インプが出撃",
    desc: "出撃インプに墓守。味方の死亡で魂を回収→アンデッド復帰時の魂の徴収へ",
    check(units, ctx) {
      const p = Synergy.pool(units, ctx);
      return p.filter(u => u.race === "インプ").length >= 3 && p.filter(u => u.tags.includes("undead")).length >= 2 && units.some(u => u.race === "インプ");
    },
