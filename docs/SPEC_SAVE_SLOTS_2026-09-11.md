...
### 3.2 書き出し／読み込み
書き出し：`<textarea readonly>` に JSON を出して「コピー」ボタン（`navigator.clipboard` が無ければ選択状態にするだけ）。ファイル保存は要らない（`file://` でも動く形）。
読み込み：`<textarea>` に貼って「このスロットへ読み込む」。壊れていればモルモの一言「読めませんデス」。
### 3.3 新規は空きスロットへ
...
- browser：タイトルで3札が見える → 空きへ新規 → 2戦進めて**リロード** → タイトルの札に「作戦3」と出て「続きから」で同じ状態（`Game.state.turn` 一致）／別の空きへ新規 → 前のスロットは残る／書き出し→削除→読み込みで復元。JS エラーなし。`run-all.sh` 全通過（`resume.js` 含む）。
...
- U1 スロット数。既定 3。
- U2 ラン中のスロット切替。既定：無し。
- U3 ファイルとしての保存（ダウンロード）。既定：無し（textarea のコピーだけ。`file://` でも動く）。
