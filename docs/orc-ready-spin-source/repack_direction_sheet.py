#!/usr/bin/env python3
"""Repack nine transparent Orc source components into crop-safe Skill cells."""
...
SOURCE = Path(__file__).with_name('orc-ready-spin-source-revision-3.png')
OUTPUT = Path(__file__).with_name('orc-ready-spin-source-repacked.png')
CELL = 418
...
def components(image: Image.Image):
    ...
    if len(found) != 9:
        raise SystemExit(f'expected nine Orc components, found {len(found)}')
