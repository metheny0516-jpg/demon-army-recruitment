...
最新選定: オーナーが鈍い打撃08/09を採用。通常物理攻撃はこの2原音に統一し交互再生。
魔法通常攻撃は attackKind で既存魔法音へ分離。以下の12音仮接続は選定前の記録。

## 現在の方針（2026-09-04）

オーナーから再度「自作合成は失敗しているので外部素材を使う」と明示。
下記の合成方式は過去の不採用案であり、今後の推奨ではない。
取得済みCC0実録から12音を `assets/sfx/recorded/` に整形し、物理攻撃・防御へ仮接続した。
攻撃者の種別で斬撃・打撃・刺突用打撃・金属に分け、命中時に鳴らす。
倍速でピッチを上げず、同時サンプルを4音に制限。試聴室で旧音との比較ができる。
波形・再生・接触同期の検証と、耳での最終評価は区別する。最終的な迫力・好みはオーナー試遊待ち。
対応表・出典は `assets/sfx/LICENSES.md` と `recorded/sources.json`。
