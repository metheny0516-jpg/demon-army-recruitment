...
`node tools/sim.js 50` を実装前後で各3回。**合格帯は置かない**（系統的に動く）。報告：平均クリア率（攻めた／待ったの内訳）、平均戦闘数、防衛戦の回数と勝率、荒らされた回数、城陥落の数。目安：平均戦闘数 15〜22、クリア率 35〜60%。「略奪4回→侵攻」が 0% なら締めすぎ、80% 超なら緩すぎ（`threshold` か `enemyTierOffset` で調整して報告）。
