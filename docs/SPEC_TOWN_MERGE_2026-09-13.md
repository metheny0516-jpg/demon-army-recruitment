...
## 2.5 先行して入れたもの（2026-09-13、Claude）
- 試遊で「毎決着、建材が2減る」→ 旧「施工」が留守番の建材産出ぶんを勝手に旧施設へ積んでいた。`st.autoBuild = false`（新規・移行済み）で止めた。
  直作りの state（テスト）は `autoBuild` が無いので今までどおり積む。Opus は 2節の撤去のときにこの flag ごと消す。
