...
このメモを読み、**残件J（味方11種×驚き・にやり・涙）を進める**。2026-09-06にゴブリン・スライム・キングスライム・コボルト・オーク・スケルトンの各3差分を制作・接続した。次はゾンビから再開する。
...
| J | **6種（ゴブリン〜スケルトン）の各3差分は本番接続済み**。`assets/monsters/events/{id}/` と `EVENT_EXPRESSIONS` へ追加する契約。残り5種×3差分 |
...
ゴブリンからスケルトンまで6種は2026-09-06に完了。残り5種15枚はすべて新規生成が必要。対象は `zombie, imp, mage, necromancer, ogre`。
