#!/usr/bin/env python3
"""Prepare Zombie frame 9 without modifying the accepted 0–8 spin cells."""
...
def normalize() -> None:
    ...
    if bounds is None:
        raise SystemExit('empty Zombie frame 9')
    crop = image.crop(bounds)
    # Existing Zombie frames occupy roughly 410px of the 480px baseline canvas.
    height = 410
    ...
    with Image.open(temporary_path) as check:
        if check.size != (512, 512) or check.convert('RGBA').getchannel('A').getbbox() is None:
            ...
            raise SystemExit('normalized Zombie frame 9 failed validation')
