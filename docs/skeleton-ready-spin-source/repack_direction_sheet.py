#!/usr/bin/env python3
"""Repack nine transparent Skeleton source components into crop-safe Skill cells."""
...
SOURCE = Path(__file__).with_name('skeleton-ready-spin-source.png')
OUTPUT = Path(__file__).with_name('skeleton-ready-spin-source-repacked.png')
CELL = 418
...
def components(image: Image.Image):
    ...
    if len(found) != 9:
        raise SystemExit(f'expected nine Skeleton components, found {len(found)}')
