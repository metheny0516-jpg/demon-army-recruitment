#!/usr/bin/env python3
"""Repack nine transparent Harpy source components into crop-safe Skill cells."""
...
SOURCE = Path(__file__).with_name("harpy-ready-spin-source.png")
OUTPUT = Path(__file__).with_name("harpy-ready-spin-source-repacked.png")
CELL = 480
...
def components(image: Image.Image):
    ...
    if len(found) != 9:
        raise SystemExit(f"expected nine Harpy components, found {len(found)}")
