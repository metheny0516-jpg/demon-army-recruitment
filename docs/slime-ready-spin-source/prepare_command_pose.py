#!/usr/bin/env python3
"""Prepare Slime frame 9 without modifying the accepted 0–8 spin cells."""
...
WORK = ROOT.parent / 'slime-ready-spin-work'
SOURCE = Path(__file__).with_name('command-pose-source.png')
TRANSPARENT = WORK / '04-transparent' / '9.png'
TARGET = ROOT / 'assets/battle/units/slime/ready-spin/9.webp'
...
def normalize() -> None:
    ...
    if bounds is None:
        raise SystemExit('empty Slime frame 9')
    crop = image.crop(bounds)
    # Existing Slime frames occupy roughly 400px of the 480px baseline canvas.
    height = 400
    ...
    with Image.open(temporary_path) as check:
        if check.size != (512, 512) or check.convert('RGBA').getchannel('A').getbbox() is None:
            ...
            raise SystemExit('normalized Slime frame 9 failed validation')
