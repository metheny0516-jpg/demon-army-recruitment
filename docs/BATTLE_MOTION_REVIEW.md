...
2026-09-04の一括制作依頼を完了。味方11種・敵10役が全身の6キーポーズ＋位置補間に対応。
敵は名前ではなく役割アイコンを割当キーにし、通常編成とvariantsの49エントリを検証。
全126透過WebPをブラウザで読込確認。遠隔は予備動作→22%発射姿→62%命中→68%戻り。
近接の38%接触、名前/HP固定、倍速・圧縮・skip・低モーション・フォールバックを維持。
全役の390/1280幅・両陣営をspeciesテストで確認。最終回帰はspecies通常/orc、
battlefield、ranged、arrival、vfx-lifecycle、art-coverage。制作記録はBATTLE_SPECIES_MOTION.md。
残件は骸骨兵idle原画の隣コマ槍先（CSSクリップ継続）、必要な中割りと実際の試遊評価。
以下は初期実装の経緯。量産保留方針は最新のオーナー依頼により解除済み。
