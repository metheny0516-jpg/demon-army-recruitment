...
(async()=>{
 ...
 await page.evaluate(()=>{
   ...
   Game.trace('ate',101,null,{});Game.trace('sparked',101,null,{});Incidents.settle(Game);Game.prepareMissions();App.render();
 });
 ...
 await page.evaluate(()=>{const st=Game.state;st.incidents.done={};st.incidents.tail=null;st.incidents.active=0;Incidents.settle(Game);UI.castle('town');});
 ...
 await page.evaluate(()=>{const st=Game.state;st.roster=[Object.assign(Game.rollApplicant('skeleton'),{uid:102})];st.traces=[];st.incidents={};Game.trace('hired',102,null,{});Game.trace('cooked',102,null,{});Incidents.settle(Game);UI.set(UI.incidentCards('B'),'result');});
 ...
 assert.deepEqual(errors,[]);console.log('✓ incidents: 作戦会議・見学者・結果・なぜ・城下町・自然発生');
