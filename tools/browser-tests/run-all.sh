...
for t in first-play daily smoke autoplay mission recruitment facility clear scene cutin effects spectacle loot-relay vfx-lifecycle battlefield ranged arrival pacing sound fanfare music mormo mormo-aside mormo-spotlight contrib retry nearmiss softlock resume tier0 reroll eventui eventcast casualty report records castle memory kpi synergy synergy-pool chainmap chain-answer spotlight brief feast mercenary kingslime retreat inheritance general skills skills-window skill-fx wipe counterattack order slots town; do
  run "$t" DUMMY=1
done
