...
// 一つの鎖の起点・初条件・決着は保護し、中間は畳み掛ける。ただし各段に止まって見える下限を置く。
const relay = [
...
assert(rhythm.items[3].duration >= Scene.CHAIN_STEP_FLOOR.hit && rhythm.items[5].duration >= Scene.CHAIN_STEP_FLOOR.other,
  '畳み掛ける段も下限より短くせず、止まって見える');
assert(rhythm.items.every(i => i.scale === 1), '連鎖の段は総尺予算で二重に縮めない');
