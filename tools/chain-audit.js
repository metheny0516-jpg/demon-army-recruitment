...
// ── 正規化APIは本体（src/core/chain.js）をそのまま使う ─────
// 監査ツール側に写しを持たない。持つと本体と静かに食い違う。
const CHAIN_SRC = fs.readFileSync(__dirname + '/../src/core/chain.js', 'utf8');
const chainCtx = { BATTLE_HAPPENINGS: null, module: { exports: {} }, console };
vm.createContext(chainCtx);
vm.runInContext(fs.readFileSync(__dirname + '/../src/data/battle_happenings.js', 'utf8'), chainCtx);
vm.runInContext(CHAIN_SRC, chainCtx, { filename: 'src/core/chain.js' });
const Chain = chainCtx.module.exports.Chain;
const summarize = timeline => Chain.summarize(timeline);
const classify = (type, data) => Chain.classify(type, data);
const CHAIN_DEF_VERSION = Chain.DEF_VERSION;
...
function depthParity(n) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    // 例外を握り潰さない。戦闘が落ちたのは検証の失敗であって、飛ばしてよい事象ではない。
    const result = env.Battle.simulate(squad, makeFoes(env, 2 + (i % 6)),
      { graveyard: true, extortionLedger: true, facilityWorks: 2 });
    battles++;
...
module.exports = { Chain, classify, summarize, load, patchBattle, scenarios,
