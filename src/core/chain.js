...
//
// ── いまの立ち位置（重要） ──────────────────────────────
// 新規ランの表示・記録・KPIはV2。保存済みV1ランはV1のまま継続する。
// 戦闘計算が返す raw の chainSummary は互換用に残し、次はまだV1のまま変更しない。
//   ・倍率（battle.js の chainDepth >= 3）
//   ・連鎖ハプニングの発火条件（battle_happenings.js の u.chainDepth >= 3）
//   ・Battle.summarizeChains() の戻り値
//
// ── 数え方の原則 ────────────────────────────────────────
...
const Chain = {
  ...
  // 新規ランが実際に記録するCHAIN値のバージョン。既存ランは state.chainDefVersion を正本にする。
  // 倍率・ハプニング・演出はこの値では切り替えず、別タスクまでraw V1を維持する。
  RECORDED_VERSION: 2,
