...
// 正規ルートの第一幕。地図IDが達成条件。征服度で未攻略の場所を攻略済みにしない。
// 原稿: docs/STORY_ACT1_STANDARD_ROUTE_2026-09-21.md。特殊展開の追加は次段階。
const STORY_ROUTE = [
  { id: "mine", chapter: 2, title: "奪われた鉱山", place: "t06", goal: "鉱山を解放する", hint: "占領軍を退け、働く者たちを解放する。贈って協力を得る道も残る。" },
  { id: "checkpoint", chapter: 3, title: "境界線", place: "h06", goal: "関所を攻略し、街道を開く", hint: "前哨の次に本戦がある。略奪だけでは関所は開かない。" },
  { id: "temple", chapter: 4, title: "勇者", place: "h09", goal: "大神殿の守備隊を退ける", hint: "避難民の退避後、街道を押さえる守備隊と戦う。" },
  { id: "revolt", chapter: 5, title: "魔族の分裂", place: null, goal: "捕虜を渡さず、報復隊を止める", hint: "作戦「報復隊を止める」を選ぶ。通常の反乱鎮圧とは別の決着。" },
  { id: "fort", chapter: 6, title: "王国遠征軍", place: "h11", goal: "王都の砦を攻略し、遠征軍の拠点を落とす", hint: "砦には将軍ガレスがいる。敵情と編成を確かめる。" },
  { id: "town", chapter: 7, title: "王都への道", place: "h10", goal: "城塞都市に戻り、住民の暮らしを確かめる", hint: "制圧した町の報告を受け、王都への最後の作戦へ。", visit: true },
  { id: "capital", chapter: 8, title: "王国", place: "h12", goal: "王都を制する", hint: "育てた軍団で決戦へ。城で勇者を退ける別の着地も残る。" }
];

const STORY_ROUTE_TEXT = {
  mine: {
    intro: { chapter: 2, title: "折れたつるはし", bg: "throne", text:
      "城に、折れたつるはしが届いた。柄に『帰らせろ』と刻まれている。\nモルモ「北の鉱山からデス。コボルトの土地ですが、オークも一緒に働いていたそうです」\nモルモ「逃げ出せた方も、坑内に残された方もいます。まず、そこまで道を開きましょう」" },
    pre: { chapter: 2, title: "地の底からの声", bg: "mine", cast: { goldon: "k:goldon" }, text:
      "山腹に坑口が並び、地下から槌の音が響く。王国の兵が出口を塞いでいた。\n鉱山監督ゴールドン「操業を止めれば、王都の冬が越せません」\nモルモ「ここで働く方も、冬を越すんデス」\n坑内から『冬の前に今日の飯だ！』と声が上がる。\nモルモ「魔王様。まず、出口の兵を退けましょう」" },
    post: { chapter: 2, title: "地上へ", bg: "mine", cast: { goldon: "k:goldon" }, text:
      "坑口の守りが崩れた。コボルトもオークも、人間の作業員も、目を細めながら外へ出てくる。\nモルモ「帰りたい方は、帰って構いませんヨ」\n作業員がつるはしを持ち上げた。坑口に立った魔王軍の旗へ、歓声が上がる。\n鉱山監督ゴールドン「帳簿は……持って帰らせてください」\nモルモ「未払いの給金を、皆さんに説明してからデス」\n作業員が『新しい親方か。じゃあ給料は？』と聞いた。\nモルモ「……その質問は、城でも聞きますネ」" }
  },
  checkpoint: {
    intro: { chapter: 3, title: "山の次は、道", bg: "road", text: st =>
      (st.story.route.completed.mine ? "鉱山を出た荷車が、荷を積んだまま戻ってきた。関所で通行を拒まれたという。" : "関所の両側に、荷車と旅人が並んでいる。街道は王国軍に封鎖されている。")
      + "\nモルモ「土地を取り返しても、道が閉じたままでは困りますネ」\nモルモ「関所の書庫に、境界盟約の写しがあるそうデス。道を開いて、約束を確かめましょう」" },
    pre: { chapter: 3, title: "門の向こう", bg: "checkpoint", text:
      "石の門の向こうに、動けなくなった荷車が見える。守備隊は封鎖継続の命令を掲げた。\nモルモ「約束を確かめに来ました。書庫へ通してください」\n返事の代わりに、盾の列が門を塞ぐ。\nモルモ「……この道を、開きます」" },
    post: { chapter: 3, title: "名前を変えると", bg: "checkpoint", cast: { vissel: "k:vissel" }, text:
      "関所の門が開き、止まっていた荷車が動き出した。書庫には、二人の署名が残る盟約の写しがあった。\nモルモ「知っていた約束を、ようやく紙で示せます」\n法務官ヴィッセル「王国は、先代同士の約束は効力を失ったと判断しています」\nモルモ「前の魔王様は、亡くなったと決まったわけではありませんヨ」\n法務官ヴィッセル「私には判断できません。魔王としての申し入れは、受けられません」\nモルモ「では、この門を開けた者として」\n法務官ヴィッセル「……それなら、王都へ届けます」\n後日届いた返事は、一般民の通行は認めるが、占領地は返さないというものだった。鉱山の収益が、隣国への借金の担保になっていた。\nモルモ「話は進みました。でも、兵隊は引かないんですネ」" }
  },
  temple: {
    intro: { chapter: 4, title: "剣を持たない列", bg: "temple", cast: { allen: "k:allen", el: "k:el" }, text:
      "大神殿へ、人間と魔族の避難民が向かっている。その奥には、街道を押さえる王国の守備隊がいた。\n聖女エル「怪我人はこちら。武器は置いて」\n兵が魔族を指さした。\n聖女エル「見れば分かります。怪我も見れば分かるでしょう」\nモルモ「あの方が、勇者アレン。避難民の前に立っています」" },
    pre: { chapter: 4, title: "守るもの", bg: "temple", cast: { allen: "k:allen", el: "k:el" }, text:
      "勇者アレン「ここを渡したら、次は人の町だ」\nモルモ「渡さなくても、こちらの暮らしは困ったままデス」\n勇者アレン「……避難する人が出るまで、待ってくれ」\n魔王軍は足を止めた。最後の荷車が動き、アレンとエルは護衛についていく。\n道を塞いでいる守備隊は、まだ武器を下ろさない。\nモルモ「人々は離れました。ここからは、守備隊との戦いデス」" },
    post: { chapter: 4, title: "空いた神殿", bg: "temple", text:
      "守備隊を退け、大神殿の道が開いた。避難した人々の姿は、もうない。\nモルモ「勇者とは、まだ決着がついていません。でも、この道は開きましたヨ」\n帰還の途中、魔族の使いが追いついてきた。『なぜ人間を逃がした。捕虜を渡せ』という要求だった。\nモルモ「……次に話す相手は、私たちの側にいるようデス」" }
  },
  revolt: {
    intro: { chapter: 5, title: "同じ旗の下で", bg: "gate", cast: { radical: "n:radical" }, text: st =>
      "城門の外に、魔族の松明が並んだ。捕虜を渡せ、人間の村を焼けという。\n"
      + (st.story.flags.villageLost ? "松明の魔族「あんたは来た。来て、負けた。今度は俺たちが行く」" : "松明の魔族「村を守るというなら、奪われた分を取り返してくれ」")
      + "\nモルモ「救うために、別の村を焼くんデスか」\n松明の魔族「じゃあ、この怒りはどこへ持っていけばいい」\nモルモ「作戦会議に『報復隊を止める』を用意しました。捕虜を渡さず、止めるための出撃デス」" },
    pre: { chapter: 5, title: "門を越えさせない", bg: "gate", cast: { radical: "n:radical" }, text:
      "魔王軍は外門の前に並んだ。投降を呼びかけても、一団は松明を下ろさない。\n松明の魔族「同じ魔族を止めるのか」\nモルモ「はい。ここからは、通せません」\n武器を持つ者たちが前へ出た。自分たちの旗の意味を、自分たちで決める戦いになる。" },
    post: { chapter: 5, title: "翌朝の列", bg: "gate", text:
      "報復隊の抵抗は崩れた。捕虜は渡さなかった。城門から、人間の村へ向かう松明の列は消えた。\nモルモ「怒りまで、消えたわけではありませんネ」\n翌朝、門前にまた一人来た。今日は武器を持たず、紙を握っている。\n応募に来たのだという。門衛が武器を預けるように言いかけて、言い直した。\n『履歴書は、そのまま持っていろ』\nモルモ「……面接は、します。採るかどうかは、それからデス」" }
  },
  fort: {
    intro: { chapter: 6, title: "旗は三列", bg: "camp", cast: { graham: "k:graham" }, text:
      "斥候が王国の本隊を見つけた。旗は三列。街道の向こうまで、補給の荷車が続いている。\n将軍グラハムの布告には、『この遠征で終わらせる』とあった。\nモルモ「王都の砦が、出撃の拠点になっています。あそこを落とせば、この道を押さえられます」\nモルモ「門を守るだけだった軍では、もうありませんヨ。集めたんデス、ひとりずつ」" },
    pre: { chapter: 6, title: "王都の砦", bg: "camp", text:
      "砦の上に王国旗。何度も手入れされた盾が、門の前に並ぶ。\nモルモ「敵情を確かめましたか。ここを越えれば、王都への道です」\n自軍の旗が前へ進む。今度は、こちらが城門を見上げている。" },
    post: { chapter: 6, title: "旗を下ろす音", bg: "camp", text:
      "王都の砦を制した。王国旗を下ろす音のあと、兵たちが長く息を吐いた。\n運び出そうとした大きな椅子が、戸口につかえている。\nモルモ「置いていって大丈夫デス。勝った証拠は椅子ではありません」\n椅子は置かれた。今度は旗竿がつかえた。\nモルモ「……それは傾けてください」\n街道の向こうに、王都が見える。" }
  },
  town: {
    intro: { chapter: 7, title: "征服の翌朝", bg: "town", text:
      "城塞都市の報告が届いた。守備を引き受けた町にも、朝が来る。\nモルモ「ここで暮らす方たちがいます。王都へ向かう前に、話を聞きましょう」" },
    post: { chapter: 7, title: "どこに立てばいい", bg: "town", cast: { allen: "k:allen" }, text: st =>
      "町の警備兵に、店主が近づいた。『入口だと客が怖がる』。兵が下がる。『もう少し』。角を曲がる。『そこだと見えない』。\nモルモ「守る仕事も、難しいんですネ」\n店主が腕章を持ってきた。まず、警備の者だと分かるようにすることになった。\n"
      + ((st.missionCounts?.raid || 0) ? "使いとして来たアレンは、魔王軍を恐れる町の者の話にも耳を傾けた。\n勇者アレン「この町で見たことは、伝える。怖がっている人がいることも」" : "停戦の使いとして来たアレンが、町の様子を見ている。\n勇者アレン「この町で見たことは、伝える」")
      + "\nモルモ「それでも、戦うんデスか」\n勇者アレン「王都にも、守りたい人がいる」\n丘の向こうに都が見える。最初の地図では、遠い点だった場所だ。" }
  },
  capital: {
    intro: { chapter: 8, title: "最後の作戦会議", bg: "capital", text:
      "王都が目の前にある。モルモは最初の募集紙を、もう一度折り直した。\nモルモ「最初は、門を守る人しかいなかったんですネ」\n名簿には、ここまで働いてきた者たちの名前が並ぶ。\nモルモ「魔王様。編成を、お願いしますデス」" },
    pre: { chapter: 8, title: "門が開くまで", bg: "capital", cast: { allen: "k:allen" }, text:
      "勇者アレン「僕が退けば、門が開く」\nモルモ「はい」\n勇者アレン「……なら、ここで止める」\n王都の門の前に、自分たちの軍団が並ぶ。最後は、この者たちの戦いだ。" },
    ending: { chapter: 8, title: "では、本日の業務を", bg: "capital", cast: { king: "k:king" }, text: st =>
      "王都に魔王軍の旗が立った。名ばかりだった魔王が、自分の軍団で、ここまで来た。\n国王レオンハルト四世「取り返しに来る者はいないと、思った」\nモルモ「いましたヨ」\nモルモは、自軍の方を振り返った。\n"
      + ((st.fallenTotal || 0) > 0 ? "名簿には、戻らなかった者の名前も残っている。勝ったことを、忘れたことにはしない。" : "モルモは名簿を閉じて、ようやく笑った。")
      + "\n翌朝、玉座の前には、働きたい者、助けてほしい者、文句を言いたい者が並んでいた。人間も、魔族もいる。\nモルモ「魔王様。この国の明日を、どうしましょうか」\n相談者が、片方開かない城門の話を始めた。\nモルモ「……では、本日の業務を始めましょうか」\nその列の後ろに、隣国の紋章を掲げた使者が立っている。鉱山の権益について話があるという。" }
  },
  defense: {
    choice: { chapter: 8, title: "守った先にあるもの", bg: "gate", text:
      "勇者を退け、魔王城を守り切った。\nモルモ「勝ちましたヨ！ ……あ、勝利の横断幕が裏返しデス」\n皆が見上げる旗の向こうに、王都への道が続いている。\nモルモ「このまま王都へ進みますか。それとも、この勝利を区切りに、次の戦いに備えますか」\n王都へ進むなら、第一幕の作戦を続けられる。勝利を宣言するなら、城を守った結末を迎え、第二幕へ進む。",
      choices: [{ id: "continue", label: "王都への進軍を続ける" }, { id: "advance", label: "勝利を宣言し、第二幕へ" }] },
    ending: { chapter: 8, title: "この城に立つ旗", bg: "gate", text:
      "勇者を退けた。魔王城の門の上には、自分たちの旗が残っている。\nモルモ「守りましたヨ。皆さんで、この城を」\n王都はまだ向こうにある。国王を捕らえたわけでも、王国を併合したわけでもない。それでも、この軍を無視できる者はいなくなった。\n後日、王国から使者が来た。隣国の軍も動いているという。\nモルモ「魔王様。ここから先を、どうしましょうか」\n門衛が、また応募者が来たと告げる。\nモルモ「……では、本日の業務を始めましょうか」" }
  }
};
