...
const EVENTS = [
...

  {
    ...
    title: "人間の商人",
    weight: 3,
    check(st) { return st.gold >= 50 && Game.departmentRoster("construction").length > 0; },
    cast(st) {
      const actor = EV_PICK(Game.departmentRoster("construction"), "kobold");
      return actor ? { actor: actor.uid } : null;
    },
    text(st, c) {
...

  {
    ...
    title: "雑草のスープ",
    weight: 5,
    check(st) { return st.food < 3 && Game.departmentRoster("life").length > 0; },
    cast(st) {
      const actor = EV_PICK(Game.departmentRoster("life"), "slime");
      return actor ? { actor: actor.uid } : null;
    },
    text(st, c) {
...

  {
    ...
    title: "配属換えの直訴",
    weight: 4,
    check(st) { return Game.departmentRoster("construction").length >= 2; },
    cast(st) {
      const actor = EV_PICK(Game.departmentRoster("construction"), "imp");
      return actor ? { actor: actor.uid } : null;
    },
    text(st, c) {
    ...
    options: [
      ...
      },
      {
        ...
        apply(st, c) {
          c.actor.loyalty = U.clamp(c.actor.loyalty + 10, 0, 100);
          c.other.loyalty = U.clamp(c.other.loyalty - 20, 0, 100);
          Game.oweDebt({ kind: "loyalty_dept", dept: "life", amount: -15, battlesLeft: 3,
...

  {
    ...
    title: "魔王様の織物",
    weight: 4,
    check(st) { return st.gold >= 30 && Game.departmentRoster("construction").length > 0; },
    cast(st) {
      const actor = EV_PICK(Game.departmentRoster("construction"), "goblin");
      return actor ? { actor: actor.uid } : null;
    },
    text(st, c) {
    ...
    options: [
      ...
      },
      {
        ...
        apply(st) {
          st.gold += 20;
          Game.oweDebt({ kind: "loyalty_dept", dept: "construction", amount: -15, battlesLeft: 3,
