...
- Web Audio APIによる効果音20種、音量・ミュート設定（外部音源なし）
...
`src/ui/sound.js` に音声処理を集約。Web Audio APIで攻撃・魔法・撃破・蘇生・防御・
ラウンド・シナジー・不祥事・最終決戦・勝敗・昇進など20種を合成する。
最初のユーザー操作でAudioContextを解禁し、音量とミュートはLocalStorageへ保存する。
外部音源はまだ使わない。実素材へ交換する場合も `Sound.cue()` の名前を契約として保つこと。
