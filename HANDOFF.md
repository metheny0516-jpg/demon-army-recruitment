...
### 2026-09-20：戦後クリック送り・採用案内・戦果整理（CodeX作業枝）

- `codex/report-readability`。本線 baaeab5（本人台詞統合済み）から作成。
- 戦後発表はクリック／タップ待ち。一人ごとに一言→成長→新技、最後に報酬。結果へスキップは維持。
- 採用案内を現行の詳細画面に合わせ、無料1枠と追加採用を区別。
- 結果の先頭を勝敗→報酬と給与→成長に。個人戦果と全軍一覧は details へ（中身は削除しない）。
- **総収支は未実装**：goldBefore はあるが決算後終値が記録されていない。現在残高から推定しない。確定内訳だけ表示。
- core・保存契約・数値は変更なし。単体テスト通過。ブラウザには3.3秒待っても送られない確認を追加、実行は未確認。
- 次：Opusでブラウザ回帰と実戦闘からクリック送り確認、ハプニング枝との表示変更の両立。ストーリー鉱山の作戦接続は別。
