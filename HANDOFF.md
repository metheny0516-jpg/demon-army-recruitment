...
### スライム②「増殖の元」を作業枝で実装・取り込み待ち（2026-09-16・Opus）

- **枝**：`claude/slime-arc-2`（本線 `2de9cca` から分岐。**本線へは直接 push していない**）。
  コミット：`7b6d936`（条件と決着）→ `4934472`（表示・sim・テスト）→ 結合テストと sim の注記。
- **§2 より先行できる根拠**：スライム②の経路（battle.js の `options.slimeSplit` /
  火の粉 / `summonUnit` / `result.slimeSplit`）は、§2 で落とす予定の `feastUid`・
  `flags.mercenary`・`offerOrder / orderOffers`・`retreatOffer` の**どの行とも重ならない**。
  run.js 側も `deploy()` の simOptions・`applySpiritChanges` の sparked の隣・ui/sim/テストだけで、
  削除予定の欄を読んでいない。**唯一の接点は文字列としての近さ**（simOptions リテラルに
  `offerOrder` が並ぶ／`rationContext.feastUid: null`）で、機能の依存ではない。
- **実装**：痕跡2つ（`incident slime_pond` を開いた／スライムが `sparked`）＋出撃隊にスライム、
  が揃った出撃だけ options を渡す。決着では名簿に**1体だけ**残し（満杯なら次の面接へ）、
  痕跡 `slime_spawn`・撃った者の一言 `faceLine`・結果画面の「なぜ」欄1行・`st.slimeSpawnCount` を残す。
- **検証**：node 全件通過 → browser `run-all.sh` ✓ 全通過 → `sim.js 20` 完走。
  `tools/test-slime-run.js` 24件（条件6・決着10・満杯2・**実プレイ経路の結合4**）。
- **残問題（重要）**：**sim の「分裂」列は 0 のまま**で、仕様 §2-4 の目安（1周回に1〜3回）は
  この測り方では**測れない**。原因は私の配線ではなく、**自動戦闘（`Battle.simulate`）が種族技を
  一度も出さない**こと（火球が出ない＝火の粉が飛ばない＝分裂の起点が無い）。
  実プレイの経路（`Battle.start` で火球を指示）では 20/20 で分裂が起きることを結合テストで確認済み。
  数値をいじって目安に合わせることはしていない。目安を測りたい場合は、
  sim に「指示して技を出す」経路を足す別チケットが要る。
- **次**：統括の差分レビュー → 取り込み。§2（battle.js 側）は未着手のまま。
