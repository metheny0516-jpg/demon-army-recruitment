...
2. **オーナーの試聴待ち**。各役割（剣の風切り／金属衝突／鈍い打撃）から、使いたい候補番号を3つ程度ずつ選ぶ。
...
2. **魔法結社が突出** — 第8戦の勝率が 魔法結社×5 で73%、他ビルドは4〜28%。
...
6. **敵編成の中身は攻略段階ごとに固定** — 作戦経路は分岐したが、同じ攻略段階の敵は暗記できる。
   数値に±10%の揺れを入れるだけでも違う。
7. **昇進時の姿の変化** — 能力・特殊能力・UI上の階級章は実装済み。将軍専用の立ち絵変化は未着手。
8. **敵に台詞が無い** — 勇者アレンにすら無い。最終決戦の格が上がる余地。
### 低

9. 純粋な単一種族が終盤でまだ苦しい（ゴブリン×5は第8戦6%）
10. 敵の立ち絵はまだ無い（味方は全種そろっている）
11. 当初構想の未着手分: 殿堂入り、モンスター図鑑、シナジー図鑑、実績、装備、施設、複数魔王、BGM
