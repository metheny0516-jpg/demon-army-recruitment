...
### 2026-09-20：第一幕の分かりにくい台詞・矛盾の修正（CodeX作業枝）

- `codex/story-dialogue-clarity`。src/data/story_act1.js の本文だけを修正。
- 伍長の「保証」を具体的な要求に変更。使いの回想であることを会話前に明示。
- 鉱山労働者は「帰してもらえない」に揃え、モルモが冒頭で応募者を見下す言い回しを変更。
- 関所の「黙っていた」を原本／写しの説明へ。前魔王を死亡扱いせず、王国が約束を引き継がない主張に整理。
- 最古参が必ず第一章からいたという断言を削除。抽選・効果・章進行は変更なし。
- 次／バックログ：鉱山の場所と作戦接続、斥候場面の重複抑止、国王残留／逃亡と幕末分岐は未修正。脚本枝 `codex/story-mine-script` を参照。
