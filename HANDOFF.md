...
- **差し戻し5点の対応**：
  ...
  4. **乱数は種で固定**（`U.seeded(3)`）。以前の「20/20」は保存したテストの保証ではなく、
     その場限りの実測だった。**保存したテストが保証するのは「種3で1回起きる」まで**。
     通常 sim（自動戦闘）での分裂頻度は**未検証**（下記）。
  5. §2 は**済**（本線 `922fd6a`）。ただし**自動の撤退提案（`retreat_offer`）は意図的に残されている**
     （run.js のテスト4本が入口に使っているため）。チケット §2 の追記どおり。
- **検証**：node 全件 → browser `run-all.sh` → `sim.js 20` を直列。
  `tools/test-slime-run.js` 38件（条件6・決着11・満員2・複数親10・deploy 経由の結合9）。
  `browser-tests/report.js` に即加入／面接待ち／複数親の3ケースを追加。
