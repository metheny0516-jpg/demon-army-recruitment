...
### 2026-09-19 CodeX：全技モーション設計案（実装前）

- `docs/DESIGN_SKILL_MOTIONS_2026-09-19.md`、枝 `codex/skill-motion-design`。参照本線771d4df。
- 通常攻撃19種、SKILLS全31、未割当カタログ25、自動上位6の動き・FX・失敗時・尺を整理。31＋25 IDの記載漏れゼロを照合。
- オーナー方針：通常は数歩前進＋軽い着弾。突進・跳躍・静止暗転・飛び道具を技ごとに差別化。ロマサガの提示例は動きの参考で、新技や数値変更の承認ではない。
- 次にやること／バックログ：オーナーと各技案を調整→通常攻撃＋振り下ろす／突進／一刀／火球／吸血を試作・比較→他技へ展開。必要な追加ポーズも文書化。画像制作は未着手。
- 実装前にtrait・カタログの末端イベント識別を監査。情報不足なら表示メタデータ契約を別途提案。今回はコード・数値・素材・保存契約すべて変更なし。
