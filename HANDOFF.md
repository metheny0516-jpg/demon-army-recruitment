...
## 0. 第一幕の正規筋を実装（2026-09-21・CodeX、Opusブラウザ検証待ち）

本線 `406c39b` 起点、`codex/story-act1-playable-20260921`。本線未統合。
オーナーの新指示は「まず第一幕の正規筋を通し、その後に編成・選択による大きな波乱」。古い第1章のみの制作制限よりこちらを優先する。
仕様・実装差分・検証結果・Opusへの詳細手順は `docs/IMPLEMENT_STORY_ACT1_STANDARD_ROUTE_2026-09-21.md`。
採用原稿は `docs/STORY_ACT1_STANDARD_ROUTE_2026-09-21.md`。

鉱山→関所→神殿→報復隊→砦→町の帰還報告→王都を実際の土地・決着に配線。
作戦目標と隣接する経路候補を追加。前哨勝利で領土を得ていた既存不具合を修正（Story無効時にも適用）。
王都前の勇者撃退後は進軍続行／第二幕へ進む方針を選ぶ。通常能力の簡易試走5回すべてで王都到達。
保存契約は `story.route`（completed/pending/shown/defenseDecision）、作戦に `storyRoute`。方針選択は `Game.storyChoose`。詳しくは実装文書。
神殿ではアレンと対面するが本人との戦闘追加はまだ。町の統治選択・大規模な波乱分岐も次段階。
Node 99スイート中98通過、連鎖計測1件は480秒で未完了。追加ルート試験15項目通過。ブラウザは未実施、Opus担当。
`run.js` は変更したが `battle.js`・モーション・素材は変更していない。他の未承認作業枝も混ぜていない。
...
### 検証中：第一幕の正規ルート（2026-09-21・CodeX 実装／Opus 検証・**本線未統合**）

対象 `codex/story-act1-playable-20260921`（285c6b6、親は本線 406c39b）。検証枝 `claude/story-act1-verify`。
仕様＝`docs/IMPLEMENT_STORY_ACT1_STANDARD_ROUTE_2026-09-21.md`。**まだ統合も配信もしていない。**

**重点項目の検証に合格したもの**
- `node tools/test-story-route.js` 15項目通過（期待値は緩めていない）。
- Node 全件：`test-chain-measure-retry.js` のみ400秒枠で時間切れ（既知の重いテスト。失敗ではない）。他は通過。
- `sh tools/browser-tests/run-all.sh` 全テスト通過。
- 目標カードが 390px・360px とも画面内に収まる（「第2章・奪われた鉱山／鉱山を解放する／まず沼から道を開く。」）。
- **前哨勝利→本戦→領土取得**を実画面のボタン操作で確認。
  `turn8 h04 前哨 勝→領土 2+4 変化なし` / `turn9 h04 本戦 勝→3+4` /
  `turn11 h06 前哨（経路=関所）勝→領土増えず・未達成` / `turn12 h06 本戦 勝→4+4 かつ 達成=mine,checkpoint`。
- **勇者撃退後の2択**を実際の防衛戦から確認。`defenseDecision: pending` になり、
  390px の画面内に「王都への進軍を続ける」「勝利を宣言し、第二幕へ」が出る。二重タップ安全。
  `continue` は第一幕継続、`advance` は第二幕へ。**選択待ちで再読込しても pending が復元され両ボタンが再描画される。**
- **途中保存・再読込**：章達成直後に保存→再読込で 達成／pending／shown／領土／所持金／ターンが一致。
  Node側で 敗北・撤退・訓練・前哨はいずれも章を達成させないこと、旧セーブで `route` が初期化され
  未読の `ch2_open` が消え、既読の `finale` が再表示されず、既存領土が尊重されることを確認。
- **経路案内どおりの全章通し**：Story有効の新規開始から
  鉱山→関所→神殿→報復隊→砦→町→王都を**指定の順で**完走し第二幕へ到達。JSエラーなし。
  通常編成3本すべて 22ターン・21決着・領土12・王都取得・敗北0（軍団18〜20、警戒0〜3）。

**未確認（合格とは言えない）**
- オーナーの実試遊による面白さの最終判断（分担どおりこちらでは行わない）。

**sim 50 全23方策と連鎖計測（2026-09-24 完了）**
- `tools/test-chain-measure-retry.js` は**時間枠を広げれば完走・通過**（198ラン中4ランに再起、不一致0件）。
  400秒枠での「未完了」は所要時間の問題で失敗ではない。
- `node tools/sim.js 50` を本線 406c39b と検証枝で**各2本**（sim は seed を取らないため）。
  クリア率 平均 本線 97.2% → 枝 90.0%。戦闘数は力試しを除くほぼ全方策で **+1〜+3戦**。
  前哨で領土を得られなくなった修正の効果として説明がつき、2本とも同じ向き。
  大きく動いたもの（2本とも再現）：
  - 進軍の前に訓練を1回 90/84% → **34/32%**（最大の落ち込み）
  - 魔法職寄せ 100/100% → 74/88%、慎重経営 100/100% → 84/94%、安月給 84/72% → 64/66%
  - 骸骨寄せ+求人 は戦闘数が 15.6/18.9 → 63.6/55.2 と激増（クリア率は 68/84% → 96/96% で上昇）
- **本線の時点でクリア率が大半100%**であり、`CLAUDE.md` の想定帯（5〜50%）から元々外れている。
  今回の修正は帯へ近づける向きなので、この枝が帯を壊したのではない。**帯の逸脱自体は別途の課題**。

**難易度の所見**：通常能力の自動操作で3本とも無敗・同じ22ターンで王都へ届いた。
経路の進みが戦力ではなくターン進行で決まっており、**正規筋そのものは現状かなり易しい**。
前哨修正で戦闘数は増えるが、警戒0〜3・給与も破綻しない。波乱分岐を足す余地は十分ある。

**検証側の誤りとして解消したもの（実装の不具合ではない）**
1. 道中の場面中に「結果を見る」を押すと進行が壊れるように見えた → そのボタンはその間 **DOM に無く、実ユーザーには押せない**。
2. 経路の札を拾えない → `Story.routeObjective` は表示用で `id` を持たない（id を持つのは `routeStep`）。
   経路案内の札は**手前の場所へ向かう take の札**で `storyRoute` を持たない設計。照合の誤り。
3. **「649決着・領土0」は検証ボットの不具合**（実装ではない）。領土を `lands` だけ数えて `tribes` を無視し、
   可視判定に `offsetParent` を使ったため `position: fixed` の「次へ」を一度も押せていなかった。
   修正後は通常どおり最初の領土を取得する。

**今回の検証で実装の不具合は見つからなかった。「不具合未発見」と「重点項目の検証合格」は別扱いのこと。**
