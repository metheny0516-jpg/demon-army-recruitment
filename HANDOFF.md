...
### 2026-09-20：丘の斥候の反復抑止（CodeX作業枝）

- `codex/story-scout-repeat`。road_brute_charge の初回は一ラン一度。別人に入れ替わっても同じ事件を再生しない。
- 既存 charged_early / waited の保護された痕跡を参照。新しい保存キーは不要。旧セーブの既発生も抑止。
- 例外は、過去に突撃した本人が仲間の戦闘不能後に命令を待つ短い後日談を一ラン一度。実際の死亡原因だったとは断定しない。
- 候補選出で除外するので、本文だけ隠して敵が増減することはない。初回の増減・確率は維持、繰り返し分の効果は発生しなくなる（体験・バランスへの影響あり）。
- 「斥候」「……あ」を「見張り」「……一人じゃなかった」へ具体化。初回失敗時の本文と増援通知が対応。
- 検証：test-story-scout-repeat は固定60seed（成功43/失敗17）、別人・再読込・学習後日談・新周回を確認。test-story-act1 通過。ブラウザ未確認。
- 通常 sim は Story.enabled=false なのでこの頻度修正の検証にならない。上記はバランス保証ではない。
- 次：Opus で story の実画面と回帰。台詞整理枝も同じデータファイルを変更するため両方維持。鉱山接続・総収支は別タスク。
