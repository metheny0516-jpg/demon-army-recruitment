...
2. **`test-chain-measure-retry.js`「再起が1件も起きていない」**：`SIM_NO_TOWN=1`（税収なし）の測定 198ラン で、`a2dc60d` では 6ラン に再起があったが **`f00a12b` 以降は 0ラン**。**+12G で税収なしの経済でも全滅が消えた**ということ。テストは「再起が起きる前提」なので今は何も検証していない。
   直すなら測定条件（`CHAIN_SEED_BASE` を変える／ラン数を増やす）で再起が出る seed を探すか、再起の前提を外す。**ただしこれは +12G の判断材料でもある**（3節「オーナー判断待ち」の +12G へ追記）。
...
- 第一幕の +12G を採用するか（試遊で「投資できたか」）。**追記（09-17 昼）：税収なし測定 198ラン で全滅→再起が 6ラン→0ラン に消えた（`test-chain-measure-retry.js`）。「ひやひや」を残すなら +12G と難易度は一緒に決める。**候補の通常 +18／勇者 +24 は未承認。経済の他の3数値（報酬×1.4・防衛 0.6・留守番 1/4）は入れていない（税収込みの測定は 09-16 昼の項）。
...
3. オーナーの試遊の感想を聞き、+12G と難易度の判断を取る。
