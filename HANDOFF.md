...
### 2026-09-20：第一幕の台詞整理・鉱山編脚本（CodeX・設計枝）

- 枝 `codex/story-mine-script`、`docs/STORY_MINE_SCRIPT_2026-09-20.md`。コード・画像・音・数値は未変更。
- 保証／応募者評／追い出された／黙っていたの差し替え原稿、鉱山 M1〜M6（出撃前・現地・対面・戦闘・勝利・敗北）を具体化。
- 接続上の発見：t06 はコボルト部族圏だが物語は王国占領オーク鉱山。征服度だけで現地到着を語る問題、国王の残留／逃亡の不一致もある。
- **未実装／次**：本文修正 → 斥候反復抑止 → 鉱山の地図・作戦接続。実装済みと扱わない。run.js は Opus 等の一人に集約。
- **バックログ**：関所・神殿の脚本、王都攻略／勇者防衛の幕末分岐。第三幕・モルモ覚醒・確率は今回確定しない。
