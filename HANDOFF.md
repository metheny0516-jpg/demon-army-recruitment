...
## 0. 次段階の波乱分岐案（2026-09-21・CodeX、設計のみ）

`docs/DESIGN_STORY_ACT1_BRANCHES_2026-09-21.md` を追加。
正規筋 `285c6b6` からの別枝 `codex/story-act1-branch-design-20260921`。今回の差分は文書のみ。
推奨する最初の一本は「報復隊の集団面接」。現場に出した人物の働きから交渉で第5章を解決し、砦・町・王都の光景へ結果を残す。
ゲーム用会話、通常戦闘との分岐、相手の応答、人物離脱、応募、保存と二重決着の検証項目まで記載。
次の候補「動く関所」「王国軍の出張面接」は未確定。通行と領有、敵兵の所属管理を実装前に分ける必要がある。
次にやること：Opusの正規筋検証後、修正済み基点で最初の一本を実装・試遊。全案一括投入や新しい数値調整の指示ではない。
現在のコード・数値・セーブ契約は変更していない。正規筋のブラウザ検証枝へ、この設計を実装済みとして混ぜない。
