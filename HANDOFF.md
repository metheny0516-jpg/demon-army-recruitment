...
- CodeX：指示待ちの決めポーズ `ready` と防御の構え `guard`（採用できる18種×2＝36枚、`docs/SPEC_COMMAND_POSE_2026-09-15.md` §1、ブランチ `codex/command-pose-art`）。上がったら `BATTLE_SPRITES` に2ポーズ登録し `art-coverage` を 210 → 246。**その後**に Opus へ §2（battle_scene.js の配線。run.js は触らない）を貼る。
