...
### 2026-09-20：既存の本人台詞を吹き出しへ（CodeX・作業枝）

- `codex/existing-battle-voices`。オーナー訂正：「遺書」は例示。既存の戦果グラフの本人台詞を使う。
- 戦闘不能は種族の既存 `voices.dead`（第二幕・特殊種族も含む）を再利用。固定の遺書台詞を削除。
  一戦一人一回、永久戦死・召喚除外は維持。描画抽選は U.rand を消費しない。
- 戦後は確定済み contribution.voice を対象者それぞれが話す（従来の先頭一人限定を解除）。
- core・数値・保存契約は不変。台詞の新規制作なし。対応仕様書を訂正。
- 検証：test-battle-result-bubbles 通過（種族別既存台詞・特殊種族・複数人・発言除外）。ブラウザ未実施。
- 次／バックログ：Opus のハプニング枝が battle_scene.js を変更中。統合時に両変更を維持し、実画面で本人台詞・送り・飛ばすを確認。本線未マージ。
