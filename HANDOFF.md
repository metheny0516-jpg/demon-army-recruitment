...
- 第一幕の +12G を採用するか（試遊で「投資できたか」）。候補の通常 +18／勇者 +24 は未承認。経済の他の3数値（報酬×1.4・防衛 0.6・留守番 1/4）は入れていない（税収込みの測定は 09-16 昼の項）。
...
**4. 新しい統括が最初にやること（順番）**
1. Opus に「本線 adc5ff9 で node 全件・run-all・sim 20 を1回回して結果を HANDOFF に書く」。赤があればそれが最初の仕事。
