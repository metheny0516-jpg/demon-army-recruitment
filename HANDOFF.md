...
- Node 全件：`test-chain-measure-retry.js` のみ400秒枠で時間切れ（既知の282秒超の重いテスト。失敗ではない）。他は通過。
- `sh tools/browser-tests/run-all.sh` 全テスト通過。
- Story有効の新規開始で `story.route` が初期化され、即位の場面が正しい背景で出る。assets の読み込みエラー・JSエラーなし。
- 作戦会議の**目標カード**が 390px・360px とも画面内に収まる（「第2章・奪われた鉱山／鉱山を解放する／まず沼から道を開く。」）。
- Story有効の通しを1本完走し第二幕へ到達（31ターン・29決着）。ただし**案内どおりの章順ではない**。

**未確認（合格とは言えない。ここが今回の主役）**
- 鉱山→関所→神殿→報復隊→砦→町→王都を**案内に従って順に**通すこと。
- **前哨勝利→本戦の札→本戦勝利→領土取得**を実画面の操作で見ること（Nodeテストでは通過済み）。
- 勇者撃退後の2択（進軍続行／第二幕）の実画面と、選択待ちの再読込。
- 途中保存・再読込、二重タップ、撤退・敗北、旧セーブ。
- 通常編成での難易度（前哨修正による戦数増、警戒・給与への影響）、会話のテンポ・笑い。
- 通常 sim 50 の残り15方策と連鎖計測の完走。

**検証側の誤りとして解消したもの（実装の不具合ではない）**
1. 道中の場面中に「結果を見る」を押すと進行が壊れるように見えた → そのボタンはその間 **DOM に無く、実ユーザーには押せない**。検証ボットが古い参照を掴んでいた。
2. 経路の札を拾えない → `Story.routeObjective` は表示用で `id` を持たない（id を持つのは `routeStep`）。
   また経路案内の札は**手前の場所へ向かう take の札**で `storyRoute` を持たない設計。照合の誤り。

**未決の重要事項**：検証ボットで「649決着・領土0」という周回が出た。**ボットの操作ミスか実装の不具合か切り分けできていない。**
汎用ボットの長時間周回は中止し、項目ごとの短い再現テストへ切り替える方針。

**現時点で実装の不具合は未発見。ただし「不具合未発見」と「重点項目の検証合格」は別物。**
